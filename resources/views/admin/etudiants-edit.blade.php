            ...
            <tr>
                ...
                <td>
                    <img src="{{ URL::asset('img/corbeille2.png') }}" style="cursor: pointer;" 
                         data-toggle="confirmation" data-btn-ok-label="Oui" data-btn-cancel-lable="Non!"
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h1 class="etapes-titre">MODIFIER &Eacute;TUDIANT</h1>
                    </div>
                    <div class="modal-body">
                        ...
                        @endif
                        {{ Form::open(['route' => ['admin::etudiants::update', $etudiant->id], 'method' => 'post', 'id' => 'formetud']) }}
                        <div class="table-responsive" style="width: 100%;">
                            <table class="headertable" id="etudiants">
                                ...
                                <tr>
                                    <td>
                                        {{ Form::text('username', (!is_null(old('username')) ? old('username') : $etudiant->user->username), ['placeholder' => 'Login', 'style' => ($errors->has('username') ? 'border: 1px solid #A94442' : '')]) }}
                                    </td>
                                    <td>
                                        {{ Form::text('ine', (!is_null(old('ine')) ? old('ine') : $etudiant->ine), ['placeholder' => 'numero INE', 'style' => ($errors->has('ine') ? 'border: 1px solid #A94442' : '')]) }}
                                    </td>
                                    <td>
                                        {{ Form::text('nom', (!is_null(old('nom')) ? old('nom') : $etudiant->user->nom), ['placeholder' => 'Nom', 'style' => ($errors->has('nom') ? 'border: 1px solid #A94442' : '')]) }}
                                    </td>
                                    <td>
                                        {{ Form::text('prenom', (!is_null(old('prenom')) ? old('prenom') : $etudiant->user->prenom), ['placeholder' => 'Pr&eacute;nom', 'style' => ($errors->has('prenom') ? 'border: 1px solid #A94442' : '')]) }}
                                    </td>
                                    <td>
                                        {{ Form::text('email', (!is_null(old('email')) ? old('email') : $etudiant->user->email), ['placeholder' => 'Email', 'style' => ($errors->has('email') ? 'border: 1px solid #A94442' : '')]) }}
                                    </td>
                                </tr>
                            </table>
                        </div>
