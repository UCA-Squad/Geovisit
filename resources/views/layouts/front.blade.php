...
/*TODO: passer ce code dans le controleur, idem pour tout ce qui fait appel à Eloquent dans cette vue */

$interets = [];
$points = [];

if ($tpn->ateliers->count() > 0) {
    foreach ($tpn->ateliers as $atelier) {
        array_push($interets, ['point' => $atelier->id, 'image' => url('/') . $atelier->image, 'abscisse' => $atelier->timeline * 100, 'radius' => $atelier->rayon, 'adresse' => $atelier->lien_360, 'legende' => $atelier->tpns()->where('tpn_id', '=', $tpn->id)->first()->pivot->titre_atelier, 'coordX' => $atelier->x_sommaire, 'coordY' => $atelier->y_sommaire, 'site' => $atelier->site->titre, 'description' => $atelier->tpns()->where('tpn_id', '=', $tpn->id)->first()->pivot->description_atelier, 'nbExercices' => App\Models\Atelier_tpn::whereTpn_idAndAtelier_id($tpn->id, $atelier->id)->first()->exercices()->count(), 'nbExercicesFaits' => Auth::user()->exercices()->whereAtelier_tpn_id(App\Models\Atelier_tpn::whereTpn_idAndAtelier_id($tpn->id, $atelier->id)->first()->id)->count(), 'mapX' => $atelier->x_carte, 'mapY' => $atelier->y_carte, 'idtpn' => App\Models\Atelier_tpn::whereTpn_idAndAtelier_id($tpn->id, $atelier->id)->first()->id]);
        
        array_push($points, ['lat' => $atelier->x_carte, 'lon' => $atelier->y_carte]);
    }
}

$tpn_content = !empty($tpn->contenu);
?>
...
<html lang="fr">
    <head>
        ...
        <script>
            var interets = [];
            var snd_wind = new Howl({urls: ["{{URL::asset('snd/wind.mp3')}}"], loop: true, volume: 0.15, buffer: true});
            var snd_kalimba = new Howl({urls: ["{{URL::asset('snd/kalimba.mp3')}}"], loop: true, volume: 0.0});
            var dict = {!! $tpn->site->sound !!};
    ...
    <body>
        ...
        <div id="burgermenu">
            <img src="{{ URL::asset('img/burgermenu.png') }}" id="burgermenuimg"/>
        </div>
        <div id="menuicon">
        ...
        <div id="footer">
            <div id="universite">© 2016-2020 Université Clermont Auvergne & Université Pierre Marie Curie</div>
            ...
            <img id="mute" width="20px" height="20px" src="{{ URL::asset('img/sound_on.png') }}">
        </div>
        <div id="endroit">
            <div id="endroit_bloc">
                <div id="tp_endroit">{!! strtoupper($tpn->titre_tpns) !!}</div>
                <div id="nom_endroit">{!!strtoupper($tpn->site->titre)!!}</div>
                <div id="responsable_endroit">Enseignant <span id="nom_responsable_endroit">{!!$tpn->professeur->user->prenom!!} {!!$tpn->professeur->user->nom!!}</span></div>
            </div>
        </div>
        <!--Frise chronologique-->
        ...
        <div id="zoomOverlay" style="display: none;">
            ...
            <div id="zoomEndroit">
                <div id="zoomEndroitBloc">
                   ...
                   <div id="zoomSite">Site&nbsp;<span id="zoomNomSite">{{ $tpn->site->titre }}</span></div>
                </div>
            </div>            
        </div>
        <div id="chargement">
        ...
        <script>
            ...
            myVideo.src = "{{URL($tpn->site->video)}}"; 
            myVideo.play();
            ...
            myVideo.addEventListener('loadedmetadata', function() {
                ...
                function createOneCircle(i) {
                    ...
                    function chargerOverlay() {
                        ...
                        $("#oc_boutonforme").click(function () {
                            ...
                            $("#overlay_pano").load("{{url('/360')}}" + '/' + interets[i].point + '/' + interets[i].idtpn);
                        });
                ...
                $("#overlay_temp").click(function (e) {
                    if ($("#overlay_temp").css("opacity") > 0 && e.target.id !== "overlay_contenu"
                            ...
                            && !$(e.target).hasClass('exercice') && !$(e.target.parentNode).hasClass('exercice')) {
                        ...
                        if (snd_insitu === false) {
                            snd_soundscape.fadeIn(0.3, snd_fadeLength);
                            snd_kalimba.fadeIn(0.3, snd_fadeLength);
                        } else {
                            snd_wind.fadeOut(0.0, snd_fadeLength);
                            snd_kalimba.fadeOut(0.0, snd_fadeLength);
                            if (snd_360 !== undefined) {
                                snd_360.fadeIn(0.3, snd_fadeLength);
                            }
                        }
                    }
                });
                ...
                $("#icon_circle, #burgermenuimg").click(function (e) {
                    if (snd_360 !== undefined) {
                        snd_wind.fadeIn(0.3, snd_fadeLength);
                        snd_soundscape.fadeIn(0.3, snd_fadeLength);
                        snd_kalimba.fadeIn(0.3, snd_fadeLength);
                        snd_360.fadeOut(0.3, snd_fadeLength);
                        snd_360.unload();
                        snd_insitu = false;
                    }
                ...
                function chargerIntro() {
                    ...
                    if (snd_insitu) {
                        snd_360.fadeOut(0.0, snd_fadeLength);
                    } else {
                        snd_soundscape.fadeOut(0.0, snd_fadeLength);
                        snd_kalimba.fadeOut(0.0, snd_fadeLength);
                    }
                    snd_wind.fadeIn(0.3, snd_fadeLength);
                    $("#overlay_contenu").html(`{!! addslashes($tpn->contenu) !!}`); //titre intro

                    if ($("#overlay_click").height() > $("#overlay_contenu").height()) {
                        $("#scrollbarrette").css({"opacity": "0"});
                    }
                    $("#nom_site").append("{!!$tpn->site->titre!!}".toUpperCase()); //site du TPN
                ...
                function lancerCarte() {
                    ...
                    $("#overlay_temp").css({"z-index": "500000008"});
                    $("#overlay_temp").empty();
                    $("#overlay_temp").load("{{ url('/carte/' . $tpn->site->id) }}", remplirCarte);
                    tl_overlay.to($("#overlay_temp"), 0.5, {autoAlpha: 0.85});
                }
                ...
                function remplirCarte() {
                    if ({{ $tpn->site->sig_map }}) {
                        ...
                        $("#carte").attr({"src": "{{ URL::asset($tpn->site->img_mini_map) }}"}); //img carte du tpn
                        $("#sentier").attr({"src": "{{ URL::asset($tpn->site->img_sentier) }}"}); //sentier carte du tpn (si existant)
                        if (!$("#sentier").attr("src")) {
                        ...
                        for (i = 0; i < interets.length; i++) {
                            if (i === sdTemp) {
                                $("#contenu_carte").append("<div id=\"point" + i + "\"><svg style=\"position:absolute; top:" 
                                        ...
                                        + "%; transform: translate(-50%, -50%);\" height=\"50\" width=\"50\"><circle id=\"point_anim\" cx=\"25\" cy=\"25\" r=\"20\" stroke=\"#e89261\" stroke-width=\"2px\" " 
                                        + "stroke-dasharray=\"20\" stroke-opacity=\"1\" fill=\"#fa7346\" fill-opacity=\"0\" /><circle cx=\"25\" cy=\"25\" r=\"15\" fill=\"#fa7346\" fill-opacity=\"1\" /></svg></div>");
                ...
                function chargerProfil() {
                    ...
                    for (i = 1; i <= interets.length; i++) {
                        ...
                        if ((i - 1) === sdTemp) {
                            $("#contenu_ateliers").append("<div class=\"cercleStyle\" id=\"cercleAtelier" + (i - 1) + "\">"
                                    + "<svg height=\"20\" width=\"20\"><circle cx=\"10\" cy=\"10\" r=\"2\" stroke=\"none\" stroke-width=\"3\" fill=\"#fa7346\" /></svg>"
                                    + "</div>");
                            TweenLite.to($("#cercleAtelier" + (i - 1)), 2, {scale: 3, force3D: false});
                        } else {
                         $("#contenu_ateliers").append("<div class=\"cercleStyle\" id=\"cercleAtelier" + (i - 1) + "\">" 
                                 + "<svg height=\"20\" width=\"20\"><circle cx=\"10\" cy=\"10\" r=\"2\" stroke=\"none\" stroke-width=\"3\" fill=\"white\" /></svg>" 
                                 + "</div>");
                        }
                        $("#cercleAtelier" + (i - 1)).css({"transform": "translate(" + (x - 10) + "px, " + (y * ( - 1) - 10) + "px)"});
                        //console.log(208*Math.cos(0*(2*Math.PI / 3)), 208*Math.sin(0*(2*Math.PI / 3)));
                    }
                    majCercle(sdTemp);
                ...
                function myMove(e) {
                    if (dragok && compteur == -2 && $("#overlay_temp").css("opacity") == 0 && $("#overlay_pano").css("pointer-events") == "none") {
                        ...
                        var chain_index = myVideo.currentTime / myVideo.duration;
                        // Chain Index Update
                        snd_soundscape.set_pos(chain_index);
                    }
                }
                ...
                var tpn_content = {{ json_encode($tpn_content) }};
                if (tpn_content) {
                    init();
                } else {
                    init2();
                ...
                document.ontouchstart = myDown;
                document.ontouchend = myUp;
